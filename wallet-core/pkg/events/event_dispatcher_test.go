...
func (suite *EventDispatcherTestSuite) SetupTest() {
	...
	suite.event = TestEvent{Name: "test1", Payload: "test1", DateTime: time.Now()}
	suite.event2 = TestEvent{Name: "test2", Payload: "test2", DateTime: time.Now()}
}
...
func (suite *EventDispatcherTestSuite) TestEventDispatcher_Remove() { // Unregister
	// Event 1
	err := suite.eventDispatcher.Register(suite.event.GetName(), &suite.handler)
	suite.Nil(err)
	suite.Equal(1, len(suite.eventDispatcher.handlers[suite.event.GetName()]))
